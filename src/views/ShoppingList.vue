  ...
  <div>
    ...
    <div class="nav">
      ...
      <div class="option" @click="groupByType = !groupByType">
        ...
        <img class="icon" src="img/option.png"/>
      </div>
    </div>
    ...
    <div class="footer"><img class="button" src="img/btn-refrigerator.png" @click="addToRefrigerator"/></div>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    const shoppingItems = await this.$axios.get("/cabinet/userId/shopping_list");
    const recommendationList = await this.$axios.get("/cabinet/userId/recommendation_list");
    this.shoppingItems = shoppingItems.data.shoppingItems;
    this.recommendationList = recommendationList.data.recommendationList;
    this.shoppingItems.forEach(element => element.selected = false);
    this.recommendationList.forEach(element => element.selected = false);
    this.recommendationList.sort((a, b) => b.quantity - a.quantity);
    
    setInterval(async () => {
      const res = await this.$axios.get("/cabinet/userId/shopping_list");
  ...
  methods: {
    ...
    async delFood(food) {
      ...
      if (res.status === 200) {
        res = await this.$axios.get("/cabinet/userId/shopping_list");
        res.data.shoppingItems.forEach(element => {
          element.selected = this.shoppingItems.filter(
            item => item.id === element.id
          )[0].selected;
    ...
    async addToRefrigerator() {
      ...
      const res = await this.$axios.post("/cabinet/userId/buy", buyItems);
